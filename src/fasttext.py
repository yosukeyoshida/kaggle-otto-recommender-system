...
def get_item_embeddings(output_dir):
    ...
    w2vec = FastText(sentences=sentences, vector_size=32, min_count=1, workers=4, window=25, sg=1, negative=20)
...
def main(cv, output_dir, **kwargs):
    ...
    w2vec = FastText(sentences=sentences, vector_size=32, min_count=1, workers=4, window=kwargs["window"], sg=1)
    print(f"window={kwargs['window']}")
    if CFG.wandb:
...
if __name__ == "__main__":
    ...
    # run_name = None
    # if CFG.wandb:
    #     wandb.init(project="kaggle-otto", job_type="fasttext")
    #     run_name = wandb.run.name
    # if run_name is not None:
    #     output_dir = os.path.join("output/fasttext", run_name)
    # else:
    ...
    output_dir = "output/fasttext/item_embeddings"
    os.makedirs(output_dir, exist_ok=True)
