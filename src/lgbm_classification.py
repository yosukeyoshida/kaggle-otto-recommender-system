...
def main(single_fold):
    run_name = None
    if CFG.wandb:
        wandb.init(project="kaggle-otto", job_type="lgbm_classification")
        run_name = wandb.run.name
    if run_name is not None:
        output_dir = os.path.join("output/lgbm_classification", run_name)
    else:
        output_dir = "output/lgbm_classification"
    os.makedirs(output_dir, exist_ok=True)

    run_train("clicks", output_dir, single_fold)
    run_train("carts", output_dir, single_fold)
    run_train("orders", output_dir, single_fold)
