...
--     FROM `kaggle-352109.otto.covisit`
    ...
    SELECT
        ...
        NULL AS covisit_carts_candidate_num,
        NULL AS covisit_orders_candidate_num,
        ROW_NUMBER() OVER (PARTITION BY session) AS aggs_candidate_num
    FROM `kaggle-352109.otto.aggs_cv`, UNNEST(labels.list) AS list
--     FROM `kaggle-352109.otto.aggs`, UNNEST(labels.list) AS list
), union_all AS (
    SELECT
        session,
        aid,
        ...
        MAX(aggs_candidate_num) AS aggs_candidate_num,
    FROM (
        SELECT * FROM aggregate_by_session_aid
        UNION ALL
        SELECT * FROM covisit
        UNION ALL
        SELECT * FROM aggs WHERE aggs_candidate_num <= 50
    ) t
    GROUP BY session, aid
), session_stats1 AS (
    SELECT
