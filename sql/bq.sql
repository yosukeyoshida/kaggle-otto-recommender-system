EXPORT DATA
  OPTIONS(
    uri='gs://kaggle-yosuke/lgbm_dataset/20230105/train_*.parquet', -- FIXME
--     uri='gs://kaggle-yosuke/lgbm_dataset_test/20230105/test_*.parquet',
    format='PARQUET',
    overwrite=true
  )
AS
...
--     FROM `kaggle-352109.otto.covisit`
    ...
    SELECT
        ...
        NULL AS narm_candidate_num,
        NULL AS sasrec_candidate_num,
    FROM `kaggle-352109.otto.w2v_e20_cv` -- FIXME
--     FROM `kaggle-352109.otto.w2v_e20`
    WHERE aid is not NULL
), gru4rec AS (
    SELECT
        session,
